...
export class CommonProductsService {
  ...
  resetForm() {
    ...
    this.dateReleaseControl.setValue(new Date().toISOString().split('T')[0]);
    this.dateRevisionControl.setValue(new Date().toISOString().split('T')[0]);
  }

  resetProductDtoAndForm() {
    this.productDto$.next(this.INITIAL_DTO_VALUE);
    this.resetForm()
  }
  ...
  public syncDto() {
    ...
    this.productDto$.value.date_release = this.dateReleaseControl.value.split('T')[0];
    this.productDto$.value.date_revision = this.dateRevisionControl.value.split('T')[0];
  }
}
